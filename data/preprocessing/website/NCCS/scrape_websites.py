...
def get_content(soup):
    ...
    elements = results.find_all("div", class_ = "content")
    title = elements[0].find("h3").text.strip()
    parse_text = ""
...
for url in URLs:
    ...
    data_df = pd.DataFrame([[url, "NCCS: " + title, content]], columns = ['URL', 'Title', 'Content'])
    save_df = pd.concat([save_df, data_df])
    
with open("nccs_website_transcript.csv", "w") as file:
